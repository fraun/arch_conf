...
define(['jquery', 'underscore', 'be_backbone', 'etask', 'be_util', 'zerr',
    'be_browser', 'be_lib', 'escape', 'version_util', 'string',
    'be_config', 'bootstrap', 'storage', 'jquery_cookie'],
...
function get_uuid(){
    return etask({name: 'get_uuid', cancel: true}, [function(){
        ...
        return (ret.sync && ret.sync.uuid) || (ret.local && ret.local.uuid) ||
            ret.localStorage || ret.cookie || (ret.ccgi && ret.ccgi.value);
    }, function catch$(err){
...
function persist_uuid(uuid){
    return etask({name: 'persist_uuid'}, [function(){
        return etask.all({allow_fail: true}, {
            sync: chrome && be_lib.storage_sync_set({uuid: uuid}),
...
function load_config(be_ver){
    ...
    require(['be_config'], function(_be_config){
        ...
        require(['be_rmt_ext'], function(be_rmt){
            if (E.get('rmt_loaded'))
                return;
...
E.load_rmt = function(){
    ...
    var no_cache_require = require.config({context: 'no_cache',
        baseUrl: conf.url_bext, waitSeconds: 20,
